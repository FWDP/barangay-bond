...
export const ProfileSettingsPanel: React.FC<ProfileSettingsPanelProps> = ({ profile, xlmBalance, onRequestResubmission }) => {
  ...
  const duplicateHeader = isDuplicateAutoReject ? "Your profile requires full resubmission. Please resubmit the required documents for review." : "";
        ...
        <div className="panel-card" style={{ border: `1px solid ${isResubmissionRequired ? "rgba(245, 158, 11, 0.25)" : "rgba(220, 38, 38, 0.25)"}`, background: isResubmissionRequired ? "rgba(245, 158, 11, 0.03)" : "rgba(220, 38, 38, 0.03)" }}>
          ...
          <div style={{ marginTop: "1rem", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
            ...
            {profile?.resubmissionReason && (
              // Hide sensitive admin-provided reason text from resident UI. Show full notes only to admins.
              (profile.role === "system_admin" || profile.role === "barangay_admin") ? (
                ...
                <div style={{ background: "rgba(255,255,255,0.6)", border: "1px solid rgba(148,163,184,0.04)", borderRadius: "12px", padding: "0.9rem" }}>
                  <div style={{ fontSize: "0.75rem", fontWeight: 700, color: "var(--text-muted)", marginBottom: "0.25rem" }}>REVIEW NOTE</div>
                  <div style={{ fontSize: "0.9rem", color: "var(--text-primary)" }}>A staff member has provided an internal review note. Contact your Barangay Admin for details.</div>
                </div>
              )
            )}
  ...
  const renderMainWorkspace = () => {
    ...
    switch (activeMenu) {
      ...
      case "admin":
        ...
        // Approval decisions remain an admin workflow; wallet signatures are lazy/execution-level and optional.
        if (isPending || isRecoveryLocked) {
...
const AuthPage: React.FC<{ setViewState: (state: ViewState) => void; authEntryContext?: AuthEntryContext | null }> = ({ setViewState, authEntryContext }) => {
  ...
  // Hide sensitive / AI-driven rejection details from resident UI.
  // Only surface generic messages to users; preserve full details in admin logs/panels.
  const rejectionReason = rejectionMessage || authError || undefined;

  // If already authenticated and onboarding or auto-rejected, default steps and prefill values
  useEffect(() => {
    if (
      user &&
      user.emailVerified &&
      (!profile || profile.status === "onboarding" || profile.status === "inactive" || profile.verificationStatus === "auto_rejected")
    ) {
      setIsLogin(false);
      setSignUpStep(3);
      setEmail(user.email || "");
  ...
  useEffect(() => {
    if (authEntryContext && user) {
      ...
      if (profile) {
        ...
        setSelectedBarangayId(profile.requestedBarangayId || profile.barangayId || selectedBarangayId);
        setResRegion(profile.requestedRegionName || profile.barangayRegion || resRegion);
        setResProvince(profile.requestedProvinceName || profile.barangayProvince || resProvince);
        setResMunicipality(profile.requestedMunicipalityName || profile.barangayMunicipality || resMunicipality);
  ...
  const handleNextStep = () => {
    ...
    if (signUpStep === 5) {
      ...
      // Allow resident student users to leave School Name blank and default it to "N/A" later.
            ...
            <div className="rejection-card" style={{ border: "1px solid #f87171", background: "rgba(254, 226, 226, 0.8)", borderRadius: "16px", padding: "1rem", marginBottom: "1rem" }}>
              ...
              {rejectionReason ? (
                <p style={{ margin: "0.5rem 0 0", color: "#7f1d1d", fontSize: "0.88rem" }}>
                  <strong>Note:</strong> Your application requires resubmission. Please open your profile to update the requested fields or contact your Barangay Admin.
                </p>
              ) : null}
