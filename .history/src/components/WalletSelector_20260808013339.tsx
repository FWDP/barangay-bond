  ...
  const handleResetSession = async () => {
    try {
      ...
      if (profile?.walletAddress) {
        logger.debug(`[WalletSelector] Resetting profile wallet link in Firestore for UID: ${profile.uid}...`, "WalletSelector");
        await unlinkWallet();
      }

      disconnect();
      alert("Wallet session and profile wallet link cleared. You can reconnect or switch wallets and try again.");
    } catch (err: any) {
      logger.error(`[WalletSelector] Failed to clear wallet session: ${err.message}`, "WalletSelector");
      const isNoLinkError = err?.message?.includes("No wallet is currently linked");
      alert(isNoLinkError ? "Wallet session cleared. No profile wallet link was present to remove." : "Could not clear the wallet session automatically.");
    } finally {
      setCheckingLink(false);
    }
  };
