...
import { getResubmissionFieldsForPreset, getResubmissionSuggestedReasonForPreset, inferResubmissionFields, inferResubmissionPreset, type ResubmissionFieldKey, type ResubmissionPresetKey } from "../utils/reviewDecision";
...
export interface UserProfile {
  ...
  reviewOutcome?: "approved" | "rejected" | "resubmission_required";
...
interface AuthContextType {
  ...
  adminOverrideApprove: (targetUid: string, note?: string) => Promise<void>;
}
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const setDoc = async (ref: any, data: any, options?: any) => {
    ...
    try {
      if (options) {
        ...
        // If the document already exists, use merge to avoid full-overwrite permission errors
        try {
          ...
          // Fall back to writing without merge if read check fails for any reason
          await rawSetDoc(ref, data);
        }
      }
        ...
        try {
          ...
          profileUnsubscribe = onSnapshot(docRef, async (docSnap) => {
            ...
            if (userProfile.status === "inactive") {
              const reason = userProfile.verificationNotes || userProfile.resubmissionReason || userProfile.autoRejectReason || "Please contact your Barangay Administrator.";
    ...
    try {
      ...
      if (auth.currentUser) {
        if (auth.currentUser.email?.toLowerCase() === email.toLowerCase()) {
          ...
          logger.warn(`Existing auth session email (${auth.currentUser.email}) differs from registration email (${email}). Signing out stale session.`, "AuthContext", { correlationId });
          await firebaseSignOut(auth);
          logger.debug("Creating Firebase Auth credential after stale session cleanup...", "AuthContext", { correlationId });
      ...
      try {
        ...
        logger.database(`Preparing write for users/${uid} in Firestore`, "AuthContext", { correlationId });
        const userRef = doc(db, "users", uid);
        try {
          const existingSnap = await rawGetDoc(userRef);
          if (existingSnap && existingSnap.exists() && existingSnap.data().status === "inactive") {
            logger.database(`User ${uid} exists and is INACTIVE — performing restricted resubmit update`, "AuthContext", { correlationId });
            await updateDoc(userRef, {
              // Owner-editable fields for resubmission
            ...
            // New user or non-inactive existing user: full merge write
            await rawSetDoc(userRef, newProfile, { merge: true });
          }
        } catch (writeErr: any) {
          // If existence check fails for permission reasons, attempt a safe merge write
          logger.warn(`Existence check failed for users/${uid}. Falling back to merge write. Error: ${writeErr?.message ?? writeErr}`, "AuthContext", { correlationId });
          await rawSetDoc(userRef, newProfile, { merge: true });
        }
  ...
  const executeAIVerification = async (
    ...
    try {
      ...
      if (hasExactDuplicate) {
        ...
        try {
          ...
          if (forceDuplicateResubmission) {
            logger.warn(`Duplicate risk threshold reached (${dupResult.maxScore}%). Forcing full-package resubmission.`, "AuthContext", { correlationId });
            updatedProfile.verificationStatus = "auto_rejected";
            updatedProfile.status = "pending";
            updatedProfile.aiDecision = "auto_reject";
      ...
      await updateDoc(doc(db, "users", uid), {
        ...
        riskScore: updatedProfile.riskScore ?? null,
        duplicateScore: updatedProfile.duplicateScore ?? null,
  ...
  const signIn = async (email: string, password: string) => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        // Persist the last successful account resume for browser refresh or account-switch recovery.
        localStorage.setItem(AUTH_SESSION_UID_KEY, userCredential.user.uid);
        localStorage.setItem(AUTH_SESSION_EMAIL_KEY, loadedProfile.email || email);
  ...
  const signOut = async () => {
    ...
    // Persisted sign-in/session cache should be cleared when the account is intentionally signed out.
  ...
  const linkWallet = async (walletAddress: string, walletProvider: string, signedXdr: string) => {
    ...
    // Persist wallet/account binding in browser storage for strict resume on relog.
  ...
  const verifyUserInDb = async (
    ...
    const reviewAction = decision?.action || "full_reject";
    ...
    await addDoc(collection(db, "audit_logs"), {
      ...
      metadata: {
        ...
        actorUid: profile.uid,
        actorName: profile.name || profile.displayName || profile.email || "Unknown",
        reason: notes,
  ...
  const adminOverrideApprove = async (targetUid: string, note?: string) => {
    if (!user || !profile) throw new Error("User not authenticated");
    // Only barangay_admin or system_admin may perform overrides
    if (!(profile.role === "barangay_admin" || profile.role === "system_admin")) {
      throw new Error("Unauthorized: only admins may approve overrides");
    }
    ...
    const normalizeBgy = (value: any) => {
      if (typeof value !== "string") return "";
      const clean = value.trim();
      if (!clean) return "";
      const normalized = clean.toLowerCase();
      if (normalized === "n/a" || normalized === "unassigned" || normalized === "none" || normalized === "null") {
        return "";
      }
      return clean;
    };

    const adminBgyRaw = profile.barangayId || "";
    const adminBgy = normalizeBgy(adminBgyRaw);

    if (profile.role === "barangay_admin") {
      if (!adminBgy) {
        throw new Error("Unauthorized: your barangay admin account is not assigned to a valid barangay custody.");
      }

      const targetBgyRaw = targetProfile.role === "barangay_admin"
        ? (targetProfile.requestedBarangayId || targetProfile.barangayId || "")
        : (targetProfile.barangayId || targetProfile.requestedBarangayId || "");

      const targetBgy = normalizeBgy(targetBgyRaw);
      logger.debug(`adminOverrideApprove: comparing barangay ids target='${targetBgyRaw}' admin='${adminBgyRaw}'`, "AuthContext");

      if (!targetBgy) {
        throw new Error(`Unauthorized: target user has no valid barangay custody to approve (target='${targetBgyRaw}')`);
      }

      if (targetBgy !== adminBgy) {
        throw new Error(`Unauthorized: target user not in your barangay custody (target='${targetBgyRaw}' admin='${adminBgyRaw}')`);
      }
    }
    ...
    // Write audit entry
    try {
      await addDoc(collection(db, "verification_audit"), {
        userId: targetUid,
        action: "approve_override",
        byUid: user.uid,
        byName: profile.name || profile.displayName || profile.email || "Unknown",
        byRole: profile.role,
      ...
      await addDoc(collection(db, "audit_logs"), {
        ...
        actorRole: profile.role,
        targetUid,
        targetName: targetProfile.name || targetProfile.displayName || targetProfile.email || "Unknown",
        targetRole: targetProfile.role || "resident",
        barangayId: profile.barangayId || targetProfile.barangayId || "N/A",
        device: navigator.userAgent.substring(0, 100),
        timestamp: new Date().toISOString(),
        notes: `Admin override approved a ${targetProfile.verificationStatus || "inactive"} resident profile.`,
        metadata: {
          decision: "approve_override",
          targetFormerVerificationStatus: targetProfile.verificationStatus || "unknown",
          targetRequestedRole: targetProfile.requestedRole || targetProfile.role || "resident",
          approvedByUid: user.uid,
          approvedByName: profile.name || profile.displayName || profile.email || "Unknown",
          overrideNote: note || ""
        }
      });

      // Append approval to user's profile and activate account
      await updateDoc(targetRef, {
        approvals: arrayUnion(approvalEntry),
        verificationStatus: "approved",
        status: "active",
        lastDecisionBy: user.uid,
        lastDecisionAt: new Date().toISOString()
      });

      await loadUsersList();

      // Optionally, if the target is the current profile in-memory, update local state
      if (profile && targetUid === profile.uid) {
        setProfile((prev) => {
          if (!prev) return prev;
          const updated = { ...prev } as any;
          updated.verificationStatus = "approved";
          updated.status = "active";
          updated.lastDecisionBy = user.uid;
          updated.lastDecisionAt = new Date().toISOString();
          if (!Array.isArray(updated.approvals)) updated.approvals = [];
          updated.approvals = [...updated.approvals, approvalEntry];
          return updated;
        });
      }
    } catch (err: any) {
      logger.error(`adminOverrideApprove failed: ${err.message}`, "AuthContext");
      throw err;
    }
  };
