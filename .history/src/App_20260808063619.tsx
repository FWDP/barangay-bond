...
import {
  ...
  ChevronDown, ChevronRight, Activity, Bell, User, Mail, AlertCircle, RefreshCw
} from "lucide-react";
...
interface ResubmissionWorkbenchProps {
  profile: any;
  presetOverride?: ResubmissionPresetKey;
  fieldsOverride?: ResubmissionFieldKey[];
  onClose?: () => void;
  reasonOverride?: string;
  title?: string;
}

const ResubmissionWorkbench: React.FC<ResubmissionWorkbenchProps> = ({ profile, presetOverride, fieldsOverride, onClose, reasonOverride, title }) => {
  const { executeAIVerification } = useAuth();
  ...
  const preset: ResubmissionPresetKey = presetOverride
    || profile?.resubmissionPreset
    || inferResubmissionPreset(profile?.resubmissionReason || profile?.autoRejectReason || profile?.verificationNotes || "");
  const suggestedReasonSource = reasonOverride || profile?.resubmissionSuggestedReason || profile?.resubmissionReason || profile?.autoRejectReason || "";
  const displayedSuggestedReason = reasonOverride || profile?.resubmissionSuggestedReason || profile?.resubmissionReason || profile?.autoRejectReason || "Please correct the flagged identity details and resubmit.";
  const isFullPackage = preset === "full_package";
  const fields: ResubmissionFieldKey[] = Array.isArray(fieldsOverride) && fieldsOverride.length > 0
    ? fieldsOverride
    : Array.isArray(profile?.resubmissionFields) && profile.resubmissionFields.length > 0
      ? profile.resubmissionFields
      : getResubmissionFieldsForPreset(preset, inferResubmissionFields(suggestedReasonSource, ["idPhotoUrl"]));
  ...
  useEffect(() => {
    ...
    setProfilePhoto("");
    setError(null);
  }, [profile, presetOverride, fieldsOverride, suggestedReasonSource]);

  if (fields.length === 0) {
    return null;
  }

  const needsField = (fieldKey: ResubmissionFieldKey) => fields.includes(fieldKey);

  const handleImageChange = (setter: (value: string) => void) => (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const effectiveIdPhoto = idPhoto || profile?.idPhotoUrl || "";
    const effectiveSelfiePhoto = selfiePhoto || profile?.selfiePhotoUrl || "N/A";
    const effectiveProfilePhoto = profilePhoto || profile?.profilePhotoUrl || "N/A";

    if ((needsField("idPhotoUrl") || !effectiveIdPhoto) && !effectiveIdPhoto) {
      setError("Please upload the government ID photo.");
      return;
    }
    if (needsField("selfiePhotoUrl") && (!selfiePhoto || selfiePhoto === "N/A") && (!profile?.selfiePhotoUrl || profile.selfiePhotoUrl === "N/A")) {
      setError("Please upload the selfie holding your ID.");
      return;
    }
    if (needsField("profilePhotoUrl") && (!profilePhoto || profilePhoto === "N/A") && (!profile?.profilePhotoUrl || profile.profilePhotoUrl === "N/A")) {
      setError("Please upload your profile photo.");
      return;
    }

    if (needsField("name") && !fullName.trim()) {
      setError("Please correct your full name.");
      return;
    }
    if (needsField("idNumber") && !idNumber.trim()) {
      setError("Please correct your ID number.");
      return;
    }
    if (needsField("address") && !address.trim()) {
      setError("Please correct your address.");
      return;
    }
    ...
    try {
      const overrideProfile = {
        ...
        displayName: fullName.trim() || profile.displayName,
        address: address.trim() || profile.address,
        idNumber: idNumber.trim() || profile.idNumber,
        reviewOutcome: "resubmission_required",
        resubmissionPreset: preset,
        resubmissionFields: fields,
        resubmissionReason: displayedSuggestedReason,
        resubmissionSuggestedReason: displayedSuggestedReason,
      };

      await executeAIVerification(
        effectiveIdPhoto,
        effectiveSelfiePhoto,
        effectiveProfilePhoto,
        overrideProfile
      );
    } catch (err: any) {
      setError(err?.message || "Failed to submit your resubmission.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="panel-card" style={{ border: "1px solid rgba(59, 130, 246, 0.16)", background: "rgba(59, 130, 246, 0.03)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "1rem", marginBottom: "0.4rem" }}>
        <h2 className="panel-title" style={{ display: "flex", alignItems: "center", gap: "0.5rem", margin: 0 }}>
          <RefreshCw size={22} style={{ color: "var(--primary)" }} /> {title || "Resubmission Workbench"}
        </h2>
        {onClose && (
          <button type="button" className="btn btn-outline-navy btn-sm" onClick={onClose}>
            Close
          </button>
        )}
      </div>
      <p style={{ margin: 0, color: "var(--text-secondary)", fontSize: "0.9rem", lineHeight: 1.6 }}>
        {getResubmissionPresetLabel(preset)}: {getResubmissionPresetDescription(preset)}
      </p>
      {isFullPackage && (
        <div style={{ marginTop: "0.85rem", padding: "0.85rem", borderRadius: "12px", background: "rgba(245, 158, 11, 0.08)", border: "1px solid rgba(245, 158, 11, 0.18)", color: "var(--text-secondary)", fontSize: "0.85rem" }}>
          Full reject requires your full profile details and documents to be resubmitted. Email verification is not part of this correction.
        </div>
      )}
      {displayedSuggestedReason && (
        <div style={{ marginTop: "0.85rem", padding: "0.85rem", borderRadius: "12px", background: "rgba(255,255,255,0.72)", border: "1px solid rgba(148,163,184,0.2)", color: "var(--text-secondary)", fontSize: "0.85rem" }}>
          <strong>Suggested reason:</strong> {displayedSuggestedReason}
        </div>
      )}
      {error && (
        <div style={{ marginTop: "0.85rem", padding: "0.85rem", borderRadius: "12px", background: "rgba(220, 38, 38, 0.05)", border: "1px solid rgba(220, 38, 38, 0.16)", color: "#b91c1c", fontSize: "0.85rem" }}>
          {error}
        </div>
      )}
        ...
        {profile?.email && (
          <div className="form-group">
            ...
            <small style={{ color: "var(--text-muted)", marginTop: "0.35rem", display: "block" }}>
              Email cannot be changed during resubmission.
            </small>
          </div>
        )}
        {(needsField("name") || needsField("idNumber") || needsField("address")) && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "0.85rem" }}>
            {needsField("name") && (
              <div className="form-group">
                <label>Full Name</label>
                <input className="form-control" value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Update your full legal name" />
              </div>
            )}
            {needsField("idNumber") && (
              <div className="form-group">
                <label>ID Number</label>
                <input className="form-control" value={idNumber} onChange={(e) => setIdNumber(e.target.value)} placeholder="Correct the ID number" />
              </div>
            )}
            {needsField("address") && (
              <div className="form-group" style={{ gridColumn: "1 / -1" }}>
                <label>Address</label>
                <input className="form-control" value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Correct the residential address" />
              </div>
            )}
          </div>
        )}

        {(needsField("idPhotoUrl") || needsField("selfiePhotoUrl") || needsField("profilePhotoUrl")) && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "0.85rem" }}>
            {needsField("idPhotoUrl") && (
              <div className="form-group">
                <label>Government ID Photo</label>
                <input type="file" accept="image/*" className="form-control" onChange={handleImageChange(setIdPhoto)} />
              </div>
            )}
            {needsField("selfiePhotoUrl") && (
              <div className="form-group">
                <label>Selfie Holding ID</label>
                <input type="file" accept="image/*" className="form-control" onChange={handleImageChange(setSelfiePhoto)} />
              </div>
            )}
            {needsField("profilePhotoUrl") && (
              <div className="form-group">
                <label>Profile Photo</label>
                <input type="file" accept="image/*" className="form-control" onChange={handleImageChange(setProfilePhoto)} />
              </div>
            )}
          </div>
        )}
...
export const ProfileSettingsPanel: React.FC<ProfileSettingsPanelProps> = ({ profile, xlmBalance }) => {
  ...
  const isResubmissionRequired = profile?.reviewOutcome === "resubmission_required" || ((profile?.resubmissionFields?.length || 0) > 0 && profile?.status === "inactive");
  const isHardRejected = profile?.reviewOutcome === "rejected" || (profile?.status === "inactive" && !isResubmissionRequired);
  ...
  const [showResubmissionWorkbench, setShowResubmissionWorkbench] = useState(false);
  const isDuplicateAutoReject = profile?.verificationStatus === "auto_rejected" || profile?.aiDecision === "auto_reject";
  const duplicateHeader = isDuplicateAutoReject ? "Duplicate identity risk detected. Full package resubmission is required." : "";
  const [workbenchPreset, setWorkbenchPreset] = useState<ResubmissionPresetKey | undefined>(undefined);
  const [workbenchFields, setWorkbenchFields] = useState<ResubmissionFieldKey[] | undefined>(undefined);
  const [workbenchReason, setWorkbenchReason] = useState<string | undefined>(undefined);
  const [workbenchTitle, setWorkbenchTitle] = useState<string | undefined>(undefined);
  const showResubmissionButtons = true;
  ...
  const openWorkbench = (
    ...
    setWorkbenchFields(fields);
    setWorkbenchReason(reason);
    setWorkbenchTitle(title);
    setShowResubmissionWorkbench(true);
  };

  const closeWorkbench = () => {
    setShowResubmissionWorkbench(false);
    setWorkbenchPreset(undefined);
    setWorkbenchFields(undefined);
    setWorkbenchReason(undefined);
    setWorkbenchTitle(undefined);
  };

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
      {(isResubmissionRequired || isHardRejected) && (
        <div className="panel-card" style={{ border: `1px solid ${isResubmissionRequired ? "rgba(245, 158, 11, 0.25)" : "rgba(220, 38, 38, 0.25)"}`, background: isResubmissionRequired ? "rgba(245, 158, 11, 0.03)" : "rgba(220, 38, 38, 0.03)" }}>
          <h2 className="panel-title" style={{ display: "flex", alignItems: "center", gap: "0.5rem", color: isResubmissionRequired ? "var(--warning)" : "#b91c1c", marginBottom: "0.5rem" }}>
          ...
          <p style={{ margin: 0, color: "var(--text-secondary)", lineHeight: 1.6 }}>
            {isResubmissionRequired
              ? "Your account remains inside the dashboard, but only the flagged identity fields can be resubmitted until the review is cleared."
              : "Your account is locked inside the dashboard shell. You can still review your profile, but operational actions remain disabled until an administrator restores access."}
          </p>
          <div style={{ marginTop: "1rem", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
            {duplicateHeader && (
              <div style={{ background: "rgba(255,242,179,0.9)", border: "1px solid rgba(245, 158, 11, 0.2)", borderRadius: "12px", padding: "0.9rem", color: "#92400e" }}>
                <strong>Auto-reject notice:</strong> {duplicateHeader}
              </div>
            )}
            {profile?.resubmissionReason && (
              <div style={{ background: "rgba(255,255,255,0.6)", border: "1px solid rgba(148,163,184,0.2)", borderRadius: "12px", padding: "0.9rem" }}>
                <div style={{ fontSize: "0.75rem", fontWeight: 700, color: "var(--text-muted)", marginBottom: "0.25rem" }}>ADMIN REASON</div>
                <div style={{ fontSize: "0.9rem", color: "var(--text-primary)" }}>{profile.resubmissionReason}</div>
              </div>
            )}
            {isResubmissionRequired && (profile?.resubmissionFields?.length || 0) > 0 && (
              <div style={{ background: "rgba(255,255,255,0.6)", border: "1px solid rgba(148,163,184,0.2)", borderRadius: "12px", padding: "0.9rem" }}>
                <div style={{ fontSize: "0.75rem", fontWeight: 700, color: "var(--text-muted)", marginBottom: "0.4rem" }}>YOU NEED TO RESUBMIT</div>
                <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem" }}>
                ...
                {profile?.resubmissionSuggestedReason && (
                  <div style={{ marginTop: "0.75rem", fontSize: "0.85rem", color: "var(--text-secondary)" }}>
                    Suggested reason: {profile.resubmissionSuggestedReason}
                  </div>
                )}
              </div>
            )}
      ...
      {(isResubmissionRequired || isHardRejected) && showResubmissionButtons && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: "0.75rem", marginTop: "1rem" }}>
          <button
            type="button"
            className="btn btn-primary"
            onClick={() => openWorkbench("full_package", getResubmissionFieldsForPreset("full_package"), duplicateMessage, "Resubmit Full Profile")}
          >
          ...
          <button
            ...
            onClick={() => openWorkbench("custom", requiredFields, "Update only the flagged fields below.", "Resubmit Flagged Fields")}
          >
          ...
          <button
            ...
            onClick={() => openWorkbench(profile?.resubmissionPreset || "custom", requiredFields, profile?.resubmissionSuggestedReason || "Review the required corrections below.", "View Required Corrections")}
          >
            View Required Corrections
          </button>
        </div>
      )}

      {(isResubmissionRequired || isHardRejected) && showResubmissionWorkbench && (
        <ResubmissionWorkbench
          profile={profile}
          presetOverride={workbenchPreset}
          fieldsOverride={workbenchFields}
          onClose={closeWorkbench}
          reasonOverride={workbenchReason}
...
const MainLayout: React.FC<MainLayoutProps> = ({ setViewState, isGuest, setIsGuest }) => {
  ...
  const renderBannerNotice = () => {
    ...
    if (profile && profile.status === "inactive") {
      const recoveryLabel = profile.reviewOutcome === "resubmission_required" ? "Resubmission Needed" : "Account Locked";
      const recoveryMessage = profile.reviewOutcome === "resubmission_required"
        ? "Open Profile & Settings to resubmit only the flagged details."
        : "Open Profile & Settings to resubmit your full profile details and documents. Email verification is not required.";
      return (
        <div className="banner-notice bg-rose-soft border-rose text-rose-light mb-4" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "1rem" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
            <AlertCircle size={20} />
            <span>
              <strong>{recoveryLabel}:</strong> {recoveryMessage}
            </span>
          </div>
          <button className="btn btn-outline-danger btn-sm" onClick={() => setActiveMenu("profile")}>Open Profile</button>
        </div>
      );
    }
  ...
  const renderMainWorkspace = () => {
    ...
    const isPending = profile?.status !== "active";
    const isRecoveryLocked = profile?.status === "inactive";
      ...
      <div className="shell-body-layout">
        <aside className={`app-sidebar ${sidebarCollapsed ? "collapsed" : ""} ${mobileMenuOpen ? "mobile-open" : ""}`}>
          ...
          <nav className="sidebar-nav">
            ...
            {/* Projects Tab */}
            {!isGuest && (activeRole === "sk_official" || activeRole === "resident" || activeRole === "viewer") && profile?.status !== "inactive" && (
              <button
            ...
            {/* Voting Tab */}
            {!isGuest && (activeRole === "resident" || activeRole === "sk_official") && profile?.status !== "inactive" && (
              <button
            ...
            {/* Admin Console */}
            {!isGuest && (activeRole === "system_admin" || activeRole === "barangay_admin") && profile?.status !== "inactive" && (
              <button
            ...
            {/* Notifications Tab */}
            {!isGuest && profile?.status !== "inactive" && (
              <button
...
const AuthPage: React.FC<{ setViewState: (state: ViewState) => void }> = ({ setViewState }) => {
  ...
  const isRejectedUser = !!user && profile?.status === "inactive";
  const rejectionReason =
    profile?.verificationNotes ||
    profile?.autoRejectReason ||
    rejectionMessage ||
    authError ||
    undefined;
  ...
  useEffect(() => {
    if (user && user.emailVerified && (!profile || profile.status === "onboarding" || profile.status === "inactive")) {
      setIsLogin(false);
      setSignUpStep(3);
      setEmail(user.email || "");
    }
  ...
  // If the user is not authenticated and the auth page is in register mode,
  // reset the wizard to the beginning so rejected/inactive sessions do not skip step 1.
  useEffect(() => {
    if (!user && !isLogin) {
      setSignUpStep(1);
    }
    ...
    <div className="auth-layout">
      ...
      <div className="auth-card">
        ...
        <form onSubmit={handleAuth} className="panel-form">
          {isRejectedUser && (
            <div className="rejection-card" style={{ border: "1px solid #f87171", background: "rgba(254, 226, 226, 0.8)", borderRadius: "16px", padding: "1rem", marginBottom: "1rem" }}>
              <h3 style={{ margin: 0, fontSize: "1rem", color: "#b91c1c" }}>Application Review Status</h3>
              <p style={{ margin: "0.75rem 0 0", color: "#991b1b", fontSize: "0.95rem" }}>
                Your profile was rejected or flagged for resubmission. Please complete the missing or corrected information below to continue.
              </p>
              {rejectionReason ? (
                <p style={{ margin: "0.5rem 0 0", color: "#7f1d1d", fontSize: "0.88rem" }}>
...
const AppController: React.FC = () => {
  ...
  // If already logged in, redirect into the dashboard shell unless the user is still onboarding email verification.
  if (user && viewState === "landing" && profile && profile.status !== "pending_email_verification" && profile.status !== "onboarding") {
    setViewState("dashboard");
  }

  // Redirect authenticated users into the dashboard shell unless they are still onboarding email verification.
  if (user && viewState === "auth" && profile && profile.status !== "pending_email_verification" && profile.status !== "onboarding") {
    setViewState("dashboard");
  }
