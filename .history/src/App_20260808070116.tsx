...
const AuthPage: React.FC<{ setViewState: (state: ViewState) => void; authEntryContext?: AuthEntryContext | null }> = ({ setViewState, authEntryContext }) => {
  ...
  const getFirstActiveResubmissionStep = () => {
    if (!resubmissionMode) return 3;
    if (resubmissionPreset === "full_package") return 3;
    if (stepIncludesFields(3)) return 3;
    if (stepIncludesFields(4)) return 4;
    if (stepIncludesFields(5)) return 5;
    if (stepIncludesFields(6)) return 6;
    return 3;
  };
  ...
  const shouldRequireField = (field: ResubmissionFieldKey) => {
    if (!resubmissionMode) return true;
    if (resubmissionPreset === "full_package") return true;
    return resubmissionFields.includes(field);
  };

  const shouldShowField = (field: ResubmissionFieldKey) => {
    if (!resubmissionMode) return true;
    if (resubmissionPreset === "full_package") return true;
    return resubmissionFields.includes(field);
  };
  ...
  const handleNextStep = () => {
    ...
    if (signUpStep === 5) {
      ...
      if (desiredRole === "resident" && shouldRequireField("schoolName") && !schoolName.trim()) {
        setError("Please specify your school/university name.");
        return;
      }
    ...
    if (signUpStep === 6) {
      ...
      if (desiredRole === "resident" && idType === "student" && shouldRequireField("schoolName") && !schoolName.trim()) {
    ...
    <div className="auth-layout">
      ...
      <div className="auth-card">
        ...
        <p className="auth-subtitle">
          {isLogin
            ? "Access your transparency dashboard"
            : resubmissionMode
              ? "Continue your verification by updating the required fields. Email is read-only."
              : "Submit credentials to request verification role"}
        </p>
