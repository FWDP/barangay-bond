...
const AppController: React.FC = () => {
  ...
  const isReviewLocked = profile && ["pending_email_verification", "onboarding", "inactive", "pending"].includes(profile.status);
  const isAutoRejected = profile && profile.verificationStatus === "auto_rejected";
  const isAllowedDashboardUser = user && profile && !isReviewLocked && !isAutoRejected;
  const isRestrictedAuthUser = user && profile && (isReviewLocked || isAutoRejected);

  if (user && viewState === "landing") {
    if (isRestrictedAuthUser) {
      setViewState("auth");
    } else if (isAllowedDashboardUser) {
      setViewState("dashboard");
    }
  }

  if (user && viewState === "auth" && isAllowedDashboardUser) {
    setViewState("dashboard");
  }
