...
const AuthPage: React.FC<{ setViewState: (state: ViewState) => void; authEntryContext?: AuthEntryContext | null }> = ({ setViewState, authEntryContext }) => {
  ...
  const handleNextStep = () => {
    ...
    if (signUpStep === 6) {
      ...
      if (desiredRole === "resident" && idType === "student" && shouldRequireField("schoolName") && !schoolName.trim() && schoolName !== "N/A") {
...
const AppController: React.FC = () => {
  ...
  // Redirect authenticated users into the dashboard shell unless they are still onboarding, pending email verification, pending review, or in a resubmission flow.
  if (user && viewState === "auth" && profile && !["pending_email_verification", "onboarding", "inactive", "pending"].includes(profile.status)) {
    setViewState("dashboard");
  }
