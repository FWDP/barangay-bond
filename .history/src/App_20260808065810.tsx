...
type AuthEntryMode = "resubmission";
...
interface MainLayoutProps {
  ...
  onRequestResubmission: (context: AuthEntryContext) => void;
}
...
export const ProfileSettingsPanel: React.FC<ProfileSettingsPanelProps> = ({ profile, xlmBalance, onRequestResubmission }) => {
  ...
  const openResubmissionWizard = (context: AuthEntryContext) => {
...
const MainLayout: React.FC<MainLayoutProps> = ({ setViewState, isGuest, setIsGuest, onRequestResubmission }) => {
  ...
  const renderMainWorkspace = () => {
    ...
    switch (activeMenu) {
      ...
      case "profile":
        if (isGuest) return null;
        return <ProfileSettingsPanel profile={profile} xlmBalance={xlmBalance} onRequestResubmission={onRequestResubmission} />;
...
const AuthPage: React.FC<{ setViewState: (state: ViewState) => void; authEntryContext?: AuthEntryContext | null }> = ({ setViewState, authEntryContext }) => {
  ...
  useEffect(() => {
    if (authEntryContext && user) {
      ...
      if (profile) {
        ...
        setMobileNumber(profile.mobileNumber || profile.phoneNumber || "");
        setAddress(profile.address || "");
        ...
        setSelectedBarangayId(profile.barangayId || profile.brgys || selectedBarangayId);
        setResRegion(profile.region || profile.regionName || resRegion);
        setResProvince(profile.province || profile.provinceName || resProvince);
        setResMunicipality(profile.municipality || profile.municipalityName || resMunicipality);
        setProfilePhoto(profile.profilePhotoUrl || profile.profilePhoto || "");
        setIdPhoto(profile.idPhotoUrl || "");
        setSelfiePhoto(profile.selfiePhotoUrl || profile.selfiePhoto || "N/A");
      }
    }
  }, [authEntryContext, user, profile]);
...
const AppController: React.FC = () => {
  ...
  // Redirect authenticated users into the dashboard shell unless they are still onboarding email verification or in a rejected/inactive resubmission flow.
  if (user && viewState === "auth" && profile && profile.status !== "pending_email_verification" && profile.status !== "onboarding" && profile.status !== "inactive") {
    setViewState("dashboard");
  }
  ...
  switch (viewState) {
    ...
    case "auth":
      return <AuthPage setViewState={setViewState} authEntryContext={authEntryContext} />;
    case "dashboard":
      if (!user && !isGuest) {
        setViewState("landing");
        return null;
      }
      return <MainLayout setViewState={setViewState} isGuest={isGuest} setIsGuest={setIsGuest} onRequestResubmission={handleRequestResubmission} />;
    default:
      return null;
  }
};
