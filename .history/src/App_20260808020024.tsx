...
const AuthPage: React.FC<{ setViewState: (state: ViewState) => void }> = ({ setViewState }) => {
  ...
  const { signIn, signUp, signUpEmailPassword, getApprovedBarangays, user, profile, executeAIVerification, signOut } = useAuth();
    ...
    <div className="auth-layout">
      ...
      <div className="auth-card">
        ...
        <ErrorValidationModal
          ...
          actionText={error && (error.includes("already-in-use") || error.includes("already in use")) ? "Switch to Sign In" : undefined}
          onAction={error && (error.includes("already-in-use") || error.includes("already in use")) ? () => { setIsLogin(true); setSignUpStep(1); } : undefined}
        />
...
const AppController: React.FC = () => {
  ...
  // Redirect to dashboard if logged in and profile is active
