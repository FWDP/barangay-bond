...
const ResubmissionWorkbench: React.FC<ResubmissionWorkbenchProps> = ({ profile }) => {
  ...
  const preset: ResubmissionPresetKey = profile?.resubmissionPreset || inferResubmissionPreset(profile?.resubmissionReason || profile?.autoRejectReason || profile?.verificationNotes || "");
  const isFullPackage = preset === "full_package";
  const fields: ResubmissionFieldKey[] = Array.isArray(profile?.resubmissionFields) && profile.resubmissionFields.length > 0
    ? profile.resubmissionFields
    : getResubmissionFieldsForPreset(preset);
  ...
  if (profile?.status !== "inactive" || fields.length === 0) {
    return null;
  }
...
export const ProfileSettingsPanel: React.FC<ProfileSettingsPanelProps> = ({ profile, xlmBalance }) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
      ...
      {(isResubmissionRequired || isHardRejected) && <ResubmissionWorkbench profile={profile} />}
